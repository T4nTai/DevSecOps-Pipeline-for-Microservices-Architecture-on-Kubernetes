...
KUBESPRAY_REPO_DIR="${BASE_DIR}/kubespray/kubespray-src"
...
if [[ "$CLOUD" == "aws" ]]; then
  INVENTORY_FILE="${BASE_DIR}/kubespray/inventory.ini"
  log_info "Generating Kubespray inventory from Terraform outputs..."
  bash "${BASE_DIR}/kubespray/generate-inventory.sh" \
    CLOUD=aws CLUSTER="$CLUSTER" SSH_KEY="$SSH_KEY"
