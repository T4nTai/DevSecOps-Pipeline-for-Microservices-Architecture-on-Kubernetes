...
check_state() {
  if [ ! -f "$STATE_FILE" ]; then
    ...
    log_info  "Run step 01 first: bash scripts/steps/01-terraform.sh"
    exit 1
  fi
...
check_vars() {
  for var in "$@"; do
    if [ -z "${!var:-}" ]; then
      ...
      log_info  "Re-run step 01: bash scripts/steps/01-terraform.sh"
      exit 1
    fi
  done
}
...
check_k8s() {
  ...
  if [ ! -f "$KUBECONFIG" ]; then
    ...
    log_info  "Run step 02 first: bash scripts/steps/02-kubespray.sh"
    exit 1
  fi
